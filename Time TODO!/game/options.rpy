...
init -1 python hide:
    ...
    config.window_title = u"Test Experiment"
    ...
    sepiaTextBox = im.MatrixColor(
        "lines_orange.png",
        im.matrix.desaturate() * im.matrix.tint(0.9, 0.4, 0.6))
    style.window.background = Frame(sepiaTextBox, 12, 12)
...
python early:
    config.save_directory = "Test Experiment-1465478933"
...
init python:
    ...
    build.directory_name = "Test_Experiment-1.0"

    ## The name that's uses for executables - the program that users will run
    ## to start the game. For example, if this is 'mygame', then on Windows,
    ## users can click 'mygame.exe' to start the game.
    build.executable_name = "Test_Experiment"
